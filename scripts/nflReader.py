...
def setGameId(game, fantasyDB, mlbDB):
    ...
    try:
        ...
        gameTime = datetime.datetime.strptime(game.split()[1]+" "+game.split()[2], "%m/%d/%Y %I:%M%p")
        if game.split()[-1] == "ET":
            gameTime += datetime.timedelta(hours=4)
        
        with open(scoreboardPath.format(itemDate[-1], weeks[itemDate[0]+itemDate[1]] )) as fileIn:
            games = json.load(fileIn)

        for info in games["games"]:
           
            if int(info["game_time"].split()[-2].split(":")[0]) == int(gameTime.hour) and int(homeId) == int(rId(info["home_id"])) and int(awayId) == int(rId(info["away_id"])):
                gameId = info["game_id"]
                break
    except IndexError:
        pass
...
scoreboardPath = os.environ["HOME"] + "/FEFelson/NFLProjections/BoxScore/{}/{}/scoreboard.json"
...
for fileName in os.listdir(dkPath):
    ...
    if "DKSalaries" in fileName:
        ...
        gameIds = getGameIds(games, fantasyDB, nflDB)
            ...
            ##        print(row)
                    ...
                    if name != "":
                        try:
                            ...
                            playerId = getPlayerId(name, team, fantasyDB, nflDB)
