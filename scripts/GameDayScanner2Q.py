...
sumTwoCmd = "SELECT ({0[stat]}*1.0)/({0[div]}*1.0) FROM player_stats AS totals"
countGPCmd = "SELECT {0[stat]} FROM player_stats AS totals"
...
def quartile(stats):
    ...
    if size == 1:
        ...
        try:
            #print(stats[fst], stats[thd])
            result = (stats[fst]+stats[thd])/2
        except IndexError:
            pass
    #print("result",result)
...
def similarPlayers(playerId):
    players = {"minutes":[], "scoring":[], "shooting":[], "bigStats":[], "smallStats":[]}
    labels = ("minutes", "scoring", "shooting", "bigStats", "smallStats")
    statGroup = {"minutes": ["starter", "position", "min"],
                 "scoring": ["position", "fga", "fta", "points"],
                 "shooting": ["fgm", "tpa", "tpm", "points"],
                 "bigStats": ["position", "blk", "oreb", "dreb"],
                 "smallStats": ["position", "ast", "stl", "turn"]} 

    for view in ("season", "month"):

        for label in labels:
            catsCmd = "SELECT player_id, "+", ".join(["{0} AS {0}".format(stat) for stat in statGroup[label]])+" FROM player_{0} WHERE player_id = ?"
            simPlayersCmd = "SELECT pp.player_id FROM player_{0} AS pp INNER JOIN (".format(view)+catsCmd+") AS cats ON "+" AND ".join(["pp.{0}=cats.{0}".format(stat) for stat in statGroup[label]])
            #pprint(simPlayersCmd)
            for player in [x[0] for x in curs.execute(simPlayersCmd.format(view), (playerId, )).fetchall()]:
                if player not in players[label]:
                    players[label].append(player)

##    for label in labels:
##        print(label)
##        print("{} {}".format(*curs.execute("SELECT first_name, last_name FROM pro_players WHERE player_id = ? ", (playerId,)).fetchone()))
##        if len(players[label]) > 1:
##            pprint(curs.execute("SELECT first_name, last_name FROM pro_players WHERE player_id IN "+str(tuple(players[label]))).fetchall())
##        else:
##            pprint(curs.execute("SELECT first_name, last_name FROM pro_players WHERE player_id = "+str(players[label][0])).fetchall())
##        print()
    return players
...
def statEval(cmds, items):
    ...
    try:
        ...
        #print("")
        stats = curs.execute(cmds["stats"], items["stats"]).fetchall()
        ...
        itemStat = quartile(stats)
        itemUp = itemStat/quartile(statsAgainst)
...
class Player:
    ...
    def __init__(self, playerId, team, d2d=None):
        ...
        self.simPlayers = similarPlayers(self.playerId)
    ...
    def getMinProj(self, view, starter):
        ...
        simPlayers = tuple(self.simPlayers["minutes"])
        homeAwaySimilarCmd = "SELECT games.game_id, stats.player_id FROM games INNER JOIN player_stats AS stats ON games.game_id = stats.game_id AND games.{0[home]}_id = stats.team_id WHERE opp_id = ? AND "
        homeAwayAgainstCmd = "SELECT games.game_id, stats.player_id FROM games INNER JOIN player_stats AS stats ON games.game_id = stats.game_id AND games.{0[home]}_id = stats.team_id INNER JOIN (" + gamesAgainstCmd + ") AS against ON games.game_id = against.game_id AND stats.team_id = against.team_id WHERE "
        
        
        if len(simPlayers) == 1:
            homeAwaySimilarCmd += "player_id = "+str(simPlayers[0])
            homeAwayAgainstCmd += "stats.player_id = "+str(simPlayers[0])
        else:
            homeAwaySimilarCmd += "player_id IN "+str(simPlayers)
            homeAwayAgainstCmd += "stats.player_id IN "+str(simPlayers)
        ...
        items, itemCmds = makeCmds((playerCountCmd, playerCountAgainstCmd), playerAddIns, ((playerId,), (playerId, )))
        oppItems, oppCmds = makeCmds((teamCountCmd, teamCountAgainstCmd), teamAddIns, ((teamId,), (teamId, )))
        ...
        for view in ("season", "month"):
            ...
            for stat in ("fga", "fgm", "tpa", "tpm", "fta", "ftm", "oreb", "dreb", "ast", "stl", "blk", "turn", "points"):
                ...
                if stat not in ("ftm", "points"):
                    simPlayers = tuple(self.simPlayers[{"oreb":"bigStats","dreb":"bigStats","blk":"bigStats","ast":"smallStats","stl":"smallStats","turn":"smallStats", "tpa":"shooting", "tpm": "shooting", "fgm":"shooting"}.get(stat,"scoring")])
                    homeAwaySimilarCmd = "SELECT games.game_id, stats.player_id FROM games INNER JOIN player_stats AS stats ON games.game_id = stats.game_id AND games.{0[home]}_id = stats.team_id WHERE opp_id = ? AND "
                    homeAwayAgainstCmd = "SELECT games.game_id, stats.player_id FROM games INNER JOIN player_stats AS stats ON games.game_id = stats.game_id AND games.{0[home]}_id = stats.team_id INNER JOIN (" + gamesAgainstCmd + ") AS against ON games.game_id = against.game_id AND stats.team_id = against.team_id WHERE "
                    ...
                    items, itemCmds = makeCmds((playerSumCmd, playerSumAgainstCmd), playerAddIns, ((playerId,), (playerId,)))
                    oppItems, oppCmds = makeCmds((teamSumCmd, teamSumAgainstCmd), teamAddIns, ((teamId,), (teamId, )))
...
class Team:
    ...
    def __init__(self, teamId, oppId, isHome, injuries):
        ...
        for player in chain(self.starters,self.bench):
            ...
            player.scorePlayer()
            print(player)
##            for key,values in player.simPlayers.items():
##                print(key+"\n")
##                for playerId in values:
##                    print("{} {}".format(*curs.execute("SELECT first_name, last_name FROM pro_players WHERE player_id = ?",(playerId,)).fetchone()))
##                print()
...
with open(filePath.format(league, *str(gameDate).split("-"), "newSimilarProjQ"), "w") as fileOut:
    dump(data, fileOut)
