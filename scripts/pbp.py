...
##            for key, item in [(key,item) for key, item in json.loads(line)["context"]["dispatcher"]["stores"]["GamesStore"]["games"].items() if "mlb" in key and item["status_type"] == "final" and item["game_type"] != "Preseason"]:
##                pprint(item)
