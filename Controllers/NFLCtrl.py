...
from ..DB import NFLDB, NFLMatchDB
from .. import Environ as ENV
from ..Views.FelsonFrame import NFLFrame
...
class NFLCtrl(Controller):
    ...
    def setTeamStats(self):
        ...
        playerStatCmd = """
                        SELECT AVG(value)
                            FROM (SELECT SUM(value) AS value, team_id, opp_id
            ...
            for stat in teamStats1:
                ...
                try:
                    if stat not in ("3rd%", "4th%", "RuAvg", "PaAvg",):
                        ...
                        value = (info["TmRuYds"]/info["TmRuAtt"])
                except:
                    value = None
